...
1. **App 操作**（改规则/切策略组）→ ConfigEngine 修改内存模型 → 写 config.json → 合并生成 runtime-config.json → XPC 通知 Helper reload
2. **更新订阅** → 下载解析节点 → 写 proxies/{订阅名}.json → 合并生成 runtime-config.json → reload
3. **手动编辑 config.json** → FSEvents 触发 → ConfigEngine 重新加载 → 合并生成 runtime-config.json → reload → Views 刷新
4. **Helper** 通过 XPC 汇报 sing-box 状态 → AppState 更新 → Views 响应

### 数据存储

```
/Library/Application Support/BoxX/
├── config.json              ← 用户可编辑的核心配置（策略组、规则、DNS、inbound 等）
├── proxies/                 ← App 自动管理，用户不用碰
│   ├── SoCloud.json         ← 订阅源A的所有节点定义
│   ├── 良心云.json           ← 订阅源B的所有节点定义
│   └── ...
├── runtime-config.json      ← 合并后的完整配置（config.json + proxies/*），传给 sing-box
├── rules/                   ← 规则集文件（内置 + 远程下载）
└── SwiftData (App sandbox)  — 订阅 URL 列表、UI 偏好等
```

**配置拆分原则：** config.json 只包含用户手动编辑有意义的内容（策略组定义、路由规则、DNS、inbound 配置等）。订阅拉取的节点定义（vmess/ss/trojan 等）存放在 `proxies/` 目录，每个订阅源一个 JSON 文件，完全由 App 管理。

**合并流程：** ConfigEngine 在启动/保存/reload 时，将 `config.json`（核心配置）+ `proxies/*.json`（订阅节点）合并为 `runtime-config.json`，传给 sing-box。策略组的 `outbounds` 字段通过节点 tag 引用 proxies 中的节点。

**config.json 示例内容：**
- `log` / `dns` / `inbounds` / `experimental` — 完整保留
- `outbounds` — 只有 selector/urltest 策略组定义 + DIRECT/BLOCK 等特殊出站，不包含订阅节点
- `route` — 路由规则和规则集引用

这样 config.json 从 1000+ 行缩减到 100-200 行，全是用户关心的配置。
...
核心配置的 Swift 映射引擎，负责 config.json 读写 + proxies 合并。

### 职责

1. **读写 config.json** — 核心配置的 Codable 映射
2. **管理 proxies/ 目录** — 订阅节点的分文件存储
3. **合并生成 runtime-config.json** — 供 sing-box 使用的完整配置
4. **FSEvents 监听** — 检测 config.json 外部修改并自动 reload
...
@Observable
class ConfigEngine {
    private(set) var config: SingBoxConfig          // 核心配置（config.json）
    private(set) var proxies: [String: [Outbound]]  // 订阅节点（proxies/*.json，key=订阅名）

    // 核心配置读写
    func load()                       // 加载 config.json + proxies/
    func save()                       // 写回 config.json（不含订阅节点）
    func startWatching()              // FSEvents 监听 config.json 外部修改
    func stopWatching()

    // 合并 & 部署
    func buildRuntimeConfig() -> SingBoxConfig  // config + proxies 合并为完整配置
    func deployRuntime()              // 生成 runtime-config.json 并通知 Helper reload

    // 订阅节点管理
    func saveProxies(name: String, nodes: [Outbound])  // 写入 proxies/{name}.json
    func removeProxies(name: String)                     // 删除订阅源

    // 便捷操作（修改内存模型 + save + deployRuntime）
...
```
订阅 URL → 下载 → 检测格式 → 解析节点 → 写入 proxies/{订阅名}.json
                                         → 自动分组 → 更新 config.json 策略组引用
                                         → 合并生成 runtime-config.json → reload
```
...
每个订阅源独立一个 JSON 文件（`proxies/{订阅名}.json`），更新时整个文件替换，不影响其他订阅源和 config.json 中的用户配置。策略组通过 tag 引用节点，节点更新后 tag 不变则策略组选择不受影响。
