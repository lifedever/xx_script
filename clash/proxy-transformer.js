 ...
 * Changelog:
 *   1.8.0 (2026-09-24)
 *     - 删除「ℹ️ 订阅信息」组。它靠 filter 正向匹配信息节点，和 exclude-filter
 *       共用同一份关键词，机场一改节点名就把真节点收进来（这次是 DMIT 的
 *       「🇸🇬【SG】丨 福利节点（10.4到期）」）。流量 / 到期在 FlClash 的
 *       「代理页 ⋮ → 外部资源 → 订阅信息」里能看到更准的（来自订阅响应头的
 *       subscription-userinfo，不是伪节点），这个组已无存在价值
 *     - INFO_FILTER 改成「标签：值」形态匹配：除 剩余流量 / 套餐到期 /
 *       距离下次重置 / 防失联 四个专用词组外，其余关键词都要求后面跟冒号。
 *       四家机场 191 个节点全量验证：仍精确命中全部 5 类信息节点，
 *       同时放行被误杀的「福利节点（10.4到期）」，无新增误伤
 *
 *   1.7.1 (2026-09-22)
 *     - INFO_FILTER 去掉 `ISP`：它把 DMIT 的「🇬🇧【GB】英国特殊福利节点
 *       （未上公告，双ISP伪家宽，联通移动跑G口…）」当信息节点吃掉了，
 *       表现为外部资源显示 6 个代理、📦 组里只有 5 个。四家机场全量核对过，
 *       真信息节点都是 `剩余流量：x GB` / `套餐到期：…` / `【防失联】：…`
 *       这三种固定格式，不靠 ISP 识别，删掉无漏网
 *
 *   1.7.0 (2026-09-20)
...
//
// **只匹配「标签：值」形态，不要裸关键词**。裸关键词吃掉过两个真节点：
//   1.7.1  `ISP`  → 「英国特殊福利节点（未上公告，双ISP伪家宽…）」
//   1.8.0  `到期` → 「🇸🇬【SG】丨 福利节点（10.4到期）」
// 机场拿 ISP / 到期 / 流量 / 福利 这类词形容线路是常态，随时可能改名，猜不完；
// 而真信息节点的格式很固定，一律是 `剩余流量：997.62 GB` / `套餐到期：长期有效` /
// `【防失联】：putoto.lol` 这种带冒号的键值对。所以除几个专用词组外，其余关键词
// 都要求后面跟冒号。宁可漏掉一个信息节点（组里多一个不可用选项），也不要吃掉
// 一个能用的节点。改这里之前先拿实际订阅全量跑一遍新旧对比。
const INFO_FILTER =
    "(?i)剩余流量|套餐到期|距离下次重置|防失联|" +
    "(?:官网|订阅|流量|到期|重置|套餐|Traffic|Expire)\\s*[:：]";
