...
//
// 加词前先想清楚会不会撞上真节点的描述性命名：曾经的 `ISP` 就把
// 「英国特殊福利节点（双ISP伪家宽…）」整条吃掉了（见 1.7.1）。机场爱用
// ISP / 家宽 / 原生 / Premium 这类词形容线路，而真正的信息节点格式很固定
// （`剩余流量：997 GB`、`套餐到期：长期有效`、`【防失联】：xxx`），
// 宁可漏掉一个信息节点，也不要吃掉一个能用的节点
const INFO_FILTER =
    "(?i)剩余流量|套餐|Traffic|Expire|Premium|频道|订阅|流量|到期|重置|防失联|官网";
